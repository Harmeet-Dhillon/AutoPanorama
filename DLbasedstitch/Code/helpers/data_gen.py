...
def data_factory(img,p,t,patch_size, verbose=False):
    '''
    verbose: return corners as well if true
    '''

    cornersA=select_patchA(img,p,t,patch_size) # row, col
    cornersB=disturbed_patch(img,cornersA,p) # row, col
    ...
    if verbose:
        return img1, img2, H4pt, cA, cB

    return img1,img2,H4pt

def data_gen_main(img, p, t, patch_size, verbose=False):
    '''
    verbose: Return H4pt and corners if true
    '''
    img1, img2, H4pt, cA, cB = data_factory(img, p, t, patch_size, verbose=True)
    ...
    if verbose:
        return final_tensor, H4pt, cA, cB
